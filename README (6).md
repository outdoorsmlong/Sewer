...
2. **Overview tab** — dashboard view of any stage (raw / adjusted / corrected):
   depth & velocity hydrograph with dual y-axes, flow on a linked time
   axis below it, and the depth-vs-velocity scattergraph alongside — all
   on one page.
3. **Tab 1 · Review & adjust** — add additive calibration offsets for level
...
4. **Tab 2 · Correct** — the good-data set seeds automatically (drops
   ...
   - **Lasso/box-select** points directly on the scattergraph, then
     remove them from the fit, **snap them to the curve** (choose whether
     to trust the level or the velocity reading — the other value is
     reconstructed from V(L) or L(V)), or restore them. Snapped points
     show in green on the chart.
5. **Tab 3 · Results & export** — rows removed from the good set are gaps.
   Snap all gaps at once with the *trust level* / *trust velocity* bulk
   buttons (gaps without a plausible reading stay blank), or type
   individual values into the gap table; the missing value is
